#Generation of remote sensing configurations with overlapping elements
...
os.makedirs('../../../data/RemoteSensingDatasets/RemoteSensingConfigurationsPartiallyOverlapping/', exist_ok=True)
...
for inst in range(0, Ninstances):
    ...
    WorldSumForeground = np.zeros((WSize,WSize))
    WorldSumBackground = np.zeros((WSize,WSize))
    ...
    while Obj < NObj:
        ...
        Inserted = False
        if(Obj % 6 == 0):
            Inserted = insertObject(Frame, py, px, maxHalfSize, 1, WorldSumForeground, WSize, True)
        else:
            Inserted = insertObject(Frame, py, px, maxHalfSize, 1, WorldSumBackground, WSize, True)
        
        if(Inserted == True):
            insertObject(Frame, py, px, maxHalfSize, 1, WorldSum, WSize, False)
            insertObject(Frame, py, px, maxHalfSize, 1, World[Obj,:,:], WSize, False)
            Obj = Obj+1
    ...
    #Save the result
    tifffile.imsave('../../../data/RemoteSensingDatasets/RemoteSensingConfigurationsPartiallyOverlapping/Instance' + str(inst).zfill(3) + '.tiff', Result.astype(np.float32))
